...
TEST(UpdateTest, CBID_Nationality) {
    ...
    EXPECT_TRUE(db.updateCBIDNationality("Alienna", 8000) == 0);
...
TEST(EnterAlien, AD) {
    ...
    Database<string> db("CBID.txt", "CCID.txt", "CCND.txt");
    string s1 = "7183 Demelza Penley f 22 Ann Street british";
    ASSERT_TRUE(db.declareAlien("7183") == s1);

}

TEST(verifyAlienDBUpdate, AD) {

    Database<string> db("CBID.txt", "CCID.txt", "CCND.txt");
    string s1 = "7183 Demelza Penley f 22 Ann Street british";
    db.declareAlien("7183");
    //db.display();
    ASSERT_TRUE(db.verifyAlienDBUpdate("7183") == 1);
